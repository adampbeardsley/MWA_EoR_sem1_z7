...
$k_{||}$ modes. This is a result of the Lomb-Scargle periodogram interacting with the
Blackman-Harris window function we apply to the data. In the case of $k_{||}=0$
(shown as the bottom bin on the log plot), the
cosine term carries all the information and has a noise level $\sqrt{2}$ lower than any other
sine or cosine term individually. The first non-zero $k_{||}$ mode is highly covariant with the 
bottom bin due to the window function. Therefore the noise on this mode's cosine term is 
lower than other higher order cosines. With the added contribution from the sine term, this
mode contains the lowest level of noise.
}
