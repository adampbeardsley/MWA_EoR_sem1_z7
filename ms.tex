...
www.lofar.org}, \citealt{vanHaarlem:2013, Yatawatta:2013}), PAPER (Donald C. Backer Precision Array for Probing the Epoch of 
...
Furthermore, \citet{Pober:2015} and \citet{Greig:2016} were able to place constraints on physical reionization models. 
...
to exploit this foreground isolation
...
\cite[e.g.][]{Jacobs:2016, Trott:2016, Pober:2016, Dillon:2015}. All 
...
our noise \citep[B.~J.~Hazelton et al. 2016, in prep.]{Barry:2016}.
...
the data in the ``holographic frame," which has been shown to be the optimal weighting scheme for 
...
This is done to reduce the data volume, and has little impact on our results because
...
The pipeline as described up to this point is applied to both the calibrated data
...
of constant $k_{\perp}$ and $k_{||}$ (i.e. orthogonal to the $k_{||}$ axis)
...
The right axis shows the delay (Fourier dual to frequency) corresponding to the 
...
which results in non-uniform spectral sampling after gridding. This in turn
causes foreground power to mix to high spectral modes \citep{Bowman:2009}. 
The frequency dependent sampling creates a point spread function in the $k_{||}$
...
We provide axes more closely related to the measurements to assist in connecting
...
Figure~\ref{fig:example_2derror} shows the uncertainty level corresponding to the two 
...
We found that if we did not first fit and remove the polynomial terms, our reflection
...
This effect is distinct from more fundamental wedge effects 
...
catalog\footnote{KGS is an abbreviation of KATALOGSS, the KDD (Knowledge 
...
more complete GLEAM Survey \citep[][N.~Hurley-Walker et al., in review]
...
$-0.8$, the previously determined median spectral index of sources below 1.4 GHz
...
owing to a more complete sky model requiring lower amplitude gains to describe the data. 
...
images is done with uniform weighted data to minimize the effect of double instrument 
...
\citet{Lenc:2016} demonstrated the presence of strong polarized large-scale structures 
...
We compare the total residual power in the images by squaring and summing
...
The top panel shows our point source foreground model power spectrum
...
In the top panel the model power spectrum difference is almost entirely negative 
...
successfully removed 70\% of the residual power. 
...
While a +5 pointing (and beyond) is possible with the MWA, this observing campaign
...
The RTS$+$CHIPS pipeline accesses larger $k$ modes because the data is
...
As \citet{Lenc:2016} demonstrated, substantial polarized diffuse structure
...
drone-flown calibrator sources (D.~C.~Jacobs, et al. in prep). The latter method is similar to
...
Between 2013 and the time of writing, the MWA has collected over two thousand 
