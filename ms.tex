...
FHD. We test for residual flux density at source locations by calculating a fractional residual flux density RMS 
in the following way. For each snapshot 
continuum image, we find the residual flux density of all sources greater than 0.5 Jy 
subtracted within half-beam power by integrating the brightness within the source pixel.
We then scale by the integrated flux density of the sources, giving us a list of fractional
residual flux densities for the snapshot, which we then use to calculate an RMS. We found
