...
efforts to apply the analysis pipeline to a deep data integration \textbf{and compare with a reference pipeline for robust estimation}, and we discuss future work 
...
these antennas, with a tightly packed core of radius 50 m, and extending out to a radius \textbf{of}
...
give an overview below. \textbf{As a point of comparison, and to demonstrate robustness of the results, we compute the output power spectra with an independent calibration and power spectrum estimation pipeline, and demonstrate consistency.}
...
sub-band and can easily float, washing out useful information. \textbf{Please reword this sentence. It is not clear what "float" means here. Can you be more specific?}
...
\textbf{Analysis Methods for Testing Data Quality}
...
model, meaning that our calibration solutions in general are more accurate. \textbf{Consider adding a reference and a sentence about Jack's paper here. Her performed quite a few of these type of tests for understanding the impact of having an incomplete point source model.}
...
and negative, indicating that any differences are below the three hour noise level.   \textbf{Why is the region in the EoR window red here, particularly when the lower plot shows noise-like differences?}
